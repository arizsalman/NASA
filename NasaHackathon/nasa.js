...
function addMessage(sender, text, isHTML = false) {
  ...
  msg.innerHTML = isHTML
    ? (sender === "user" ? "🧑 You: " : "👨‍🚀 Astronaut says: ") + text
    : (sender === "user" ? "🧑 You: " : "👨‍🚀 Astronaut says: ") + text;
...
async function sendMessage() {
  ...
  // NASA API key (⚠️ exposed in frontend)
  const apiKey = "qzgmcOSL0mnZqi7pU5qBkzrgo6QCpuwywat5BuE7";
  ...
  if (normalizedMsg.includes("random")) {
    ...
    apiUrl += `&date=${randomDate.toISOString().split("T")[0]}`;
  }
...
function handleAstronautResponse(msg, data) {
  if (msg.includes("hi") || msg.includes("hello")) {
    ...
    addMessage(
      "astro",
      "I can answer about the picture, its title, or explain it. Try asking: 'Show me picture', 'Show me random picture', 'What is the title?', or 'Explain'."
    );
  }
}
